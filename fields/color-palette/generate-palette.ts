...
export const generatePalette = ({
  ...
  if (!validHex.test(args.accent)) return null;
  if (!validHex.test(args.gray)) return null;
  if (!validHex.test(args.background)) return null;
  ...
  const [accent9Color] = getStep9Colors(accentScaleColors, accentBaseColor);
